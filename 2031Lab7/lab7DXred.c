...
int main() {
	...
	printf("insert %d after index %d: (%d)\t", key, index, len());
	display();
...
int search(int key) {
	...
	return 0; //if doesnt exist and exits out of while loop
}
...
void insert(int d)  //  at the end
{
	...
	if (head == NULL) {/* the list is empty */
		head = newone;
		head->data = d;
		head->next = NULL; //DONT FORGET TO SET TO NULL AFTER
		//dont think i need a next
